...
- オプション入力 (Name=Valueで渡す)：
  ...
  - InitialGuess：初期解
    ...
    - 既定：$\frac{1}{n}\bm{1}$（`TargetNodes` 指定時は$m$で正規化）
    - 与えられた初期解は solver の開始時に$n$($m$)次元標準単体へ射影される．
...
その際に，full-state の場合は `gramian.computeGramian(A, T, wopts)`，ターゲット可制御性スコアの場合は `gramian.computeGramian(A, T, wopts, targetNodes)` によって必要な Gramian 列を計算する．
