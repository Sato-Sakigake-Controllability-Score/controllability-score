...
可制御性スコアは，状態ノードと入力ノードが 1:1 に対応する仮想的な枠組みを導入し，「どのノードにどの程度の重みで介入すれば系全体を最も効果的に制御できるか」を定量的に測る新しい指標である．また，可制御性スコアは「制御しやすさ」だけでなく，ノイズや外乱に対する脆弱性に関する情報も与え，どのノードがノイズに対して重要または脆弱であるかを調べる際の手がかりにもなる．

さらに，可制御性スコアの双対概念として，可観測性スコアを考えることができる．可観測性スコアは，システム

```math
\frac{dx}{dt}=Ax
```

において，どの状態ノードを観測すれば状態全体を把握しやすいかを表す指標である．これは，可制御性スコアの計算に用いるシステム行列 $`A`$ を $`A^\top`$ に置き換えることで計算できる．すなわち，本プログラムでは，例えば次のように `A.'` を入力することで可観測性スコアを計算できる．
...
- オプション入力 (Name=Valueで渡す)：
  ...
  - `WOptions`： $`W`$ 計算用オプション
    - 型　：`WOptions` scalar
    - `Method`, `Steps`, `UseScaling`, `EigTol`の入力があれば上書きされる
  ---
  - `StepSize`：最適化ステップサイズ
    - 型　：double scalar
    - 制約： $` >0 `$ 
  ...
  - `SolverOptions`：最適化用オプション
    ...
    - `StepSize`, `StepSizeInf`, `MaxIter`, `Tol`, `Rho`, `Sigma`, `Verbose`, `StoreTrace`, `SolverOptions`の入力があれば上書きされる
---
---
- 出力：
  - `p` ：VCS，AECS
...
- 追加オプション入力：
  - `TargetNodes`：ターゲットノード の添字
    - 型　：double vector
    - 制約：整数，1-indexed (重複がある場合は，実装上は出現順を保って一意化される)
---
- 出力：
  - `p`：target VCS, target AECS
    - 型　：double vector
    - 長さは `numel(unique(TargetNodes, "stable"))`
---
- 使用例：
...
本クラスは問題設定を表す． \
入力を受け取ると，`gramian.computeGramian(A, T, wopts[, TargetNodes])` によって $`W_1,\ldots,W_n`$ を計算する． \
...
- オプション入力 (Name=Valueで渡す)：
  ...
  - `TargetNodes`: ターゲットノード の添字
    - 型　：double vector
    - 制約：整数，1-indexed (重複がある場合は，実装上は出現順を保って一意化される)
    - 既定：空（full-state CS を計算）
    - 非空の場合，ターゲット可制御性スコアを計算する
  - `InitialGuess`：初期解
    - 型　：double vector
    - 制約：サイズ $`n`$ （`TargetNodes` 指定時は ターゲットノードの次元 ( $`=m`$ ) ）
    - 既定： $` \frac{1}{n}\boldsymbol{1} `$ （`TargetNodes` 指定時は $`m`$で正規化）
    - 与えられた初期解は $`n`$ ( $`m`$ )次元標準単体へ射影される．
    - 初期解が実行可能でなければ（ $`W(p)`$ が特異なら）`CSProblem.solveVcs`, `CSProblem.solveAecs`によってその値が返され，`ExitFlag=-2`となる．
---
- 注意：
...
主なプロパティとして，`WOptions`と`SolverOptions`を持つ． \
`WOptions`，`SolverOptions`のプロパティを直接変更することもできる．（4.3.1，4.3.2参照）

- 主なプロパティ：
  - `WOptions`： $`W`$ 計算用オプション
    - 型　：`WOptions` scalar
    - `Method`, `Steps`, `UseScaling`, `EigTol`への代入があれば上書きされる
  - `SolverOptions`：最適化用オプション
    - 型　：`PGSolverOptions` scalar
    - `StepSize`, `StepSizeInf`, `MaxIter`, `Tol`, `Rho`, `Sigma`, `Verbose`, `StoreTrace`, `SolverOptions`への代入があれば上書きされる
- 依存プロパティ：
  - `WOptions`と`SolverOptions`のプロパティ
    - ゲッタとセッタを持つ
---
...
- プロパティの依存関係：
  - `Method`と`Steps`
    - `Method="lyap"`のときは，`WOptions.Steps`を呼び出すと $`0`$ が返される．（内部的にはもとの数値を保持．）
    - `Method="integral"`かつ`Steps=0`で`gramian.computeGramian`を実行しようとするとエラー
  - `UseScaling`と`EigTol`
    - `UseScaling=false`のときは，`WOptions.EigTol`を呼び出すと $`0`$ が返される．（内部的にはもとの数値を保持．）
...
- プロパティ：
  - `ObjectiveValue`：終了時の目的関数値
    - 型　：double scalar
  - `Gradient`：終了時の勾配（射影勾配法であるため， $`\boldsymbol{0}`$ にはならない）
    - 型　：double vector
  ...
  - `Trace`：途中経過の情報を格納
    - 型　：struct
    - `StoreTrace=true`の場合に途中の数値情報を格納
      - `Iteration`
      - `Fval`
      - `StepNorm`
      - `Alpha`
...
- プロパティ：
  ...
  - `Sa` ：AECSの関数評価に利用する定数行列
    - 型　：cell vector
    - `Sa`は $`D^{-1}Q^{-1}Q^{-\top}D^{-\top}`$ を表す．
      ただし，目的関数評価に必要なのは $`W(p)`$ のもつブロック対角構造と同じブロック対角部分のみである．
      そこで，`Sa`はブロック対角要素を各cell要素として持つ．具体的な $`D, Q`$ の定義については, この節の最後の方で説明する．
...
また,  $`\widetilde{W}(p)`$ を通常の可制御性グラミアン， $`W(p)`$ をスケーリングされた可制御性グラミアンとし，
...
---
- アルゴリズム：
  - $`W(p)`$ の固有値分解を行い，正定値性の判定と $`\mathrm{tr}\left(W(p)^{-1}\right)`$ の計算を行う．
---
...
- 入力：
  ...
  - `targetNodes`: ターゲットノードの添字（省略可）
    - 型　：double vector
    - 制約：整数，1-indexed（重複がある場合は出現順を保って一意化される）
---
- 出力：
  - `wlist`：問題に対する可制御性グラミアンを格納した`WList`オブジェクト
    - 型　：`WList` scalar
---
- アルゴリズム：
...
- 入力：
  ...
  - `wopts`： $`W`$ 計算用オプション
    - 型　：`WOptions` scalar
- 注意：
  - 固有値の分離において`wopts.EigTol`を閾値として用いる．
  - 絶対値が`wopts.EigTol`未満の固有値は，虚軸上の固有値として判定する．
...
その際に，full-state の場合は `gramian.computeGramian(A, T, wopts)`，ターゲット可制御性スコアの場合は `gramian.computeGramian(A, T, wopts, targetNodes)` によって $`W_1,\ldots,W_n`$ を計算する．
