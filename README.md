...
加えて，arXiv:2510.13354 "Target Controllability Score" に基づく target controllability score
...
- target controllability score
  - 現在の実装では，論文の定義どおり「target node = actuator candidate node」として計算する．
  - target 版では Gramian 計算に direct な非スケーリング版を用いており，`UseScaling=true` を指定しても内部では non-scaling 計算にフォールバックする．
---
...
論文 "Target Controllability Score" における target VCS / target AECS は，
`vcs`, `aecs`, `bothcs` に `TargetNodes` オプションを与えることで計算する． \
`TargetNodes` には target node の添字ベクトルを与える．現在の実装では，論文の virtual system に合わせて，
この `TargetNodes` がそのまま actuator candidate node の集合として使われる． \
すなわち，$C$ は `TargetNodes` の成分を取り出す選択行列であり，各 $W_i(T)$ は
対応する target node への単一入力グラミアンを target 部分へ射影した
\[
W_i(T) = C W_{f,i}(T) C^\top
\]
として計算される．

- 追加オプション入力：
  - `TargetNodes`：target node の添字
    - 型　：double vector
    - 制約：整数，1-indexed
    - 重複がある場合は，実装上は出現順を保って一意化される
  - `UseScaling=true` を指定しても，target 版では内部で non-scaling 計算に切り替わる
---
- 出力：
...
本クラスは問題設定を表す． \
入力を受け取ると，full-state の場合は `gramian.computeGramian(A, T, wopts)` によって$W_1,\ldots,W_n$を計算する． \
`TargetNodes` が与えられた場合は `gramian.computeGramian(A, T, wopts, targetNodes)` によって
target output controllability Gramian の列を計算する． \
...
- オプション入力 (Name=Valueで渡す)：
  ...
  - TargetNodes：target node の添字
    ...
    - 非空の場合，target controllability score を計算する
  - InitialGuess：初期解
    - 型　：double vector
    - 制約：サイズ$n$（`TargetNodes` 指定時は target 数）
    - 既定：$\frac{1}{n}\bm{1}$（`TargetNodes` 指定時は target 数で正規化）
    - 与えられた初期解は solver の開始時に$n$次元標準単体へ射影される．
    - 初期解が実行可能でなければ（$W(p)$が特異なら）`CSProblem.solveVcs`,`CSProblem.solveAecs`によってその値が返され，ExitFlag=-2となる．
---
- 注意：
  - コンストラクタ内部で Gramian 計算を実行するため，大規模問題の場合は計算時間のボトルネックになる．
---
---
- 使用例：
...
- 入力：
  ...
  - wopts：$W$計算用オプション
    - 型　：WOptions scalar
  - targetNodes：target node の添字（省略可）
    - 型　：double vector
    - 制約：整数，1-indexed，重複なし
---
- 出力：
...
- アルゴリズム：
  ...
  - `targetNodes` が非空のときは，`wopts.Method` と `T` に応じて `gramian.infTargetLyap_`，`gramian.finTargetLyap_`，`gramian.finTargetIntegral_` のいずれかを呼び出す．
  - `targetNodes` が非空のときは，`UseScaling=true` を指定しても内部では non-scaling の target 用実装を使う．
...
その際に，full-state の場合は `gramian.computeGramian(A, T, wopts)`，target の場合は `gramian.computeGramian(A, T, wopts, targetNodes)` によって必要な Gramian 列を計算する．
