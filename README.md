...
| CSResult                | クラス | 結果の保持      |
| gramian.computeW        | 関数  | Gramian 計算 |
...
本クラスは問題設定を表す． \
入力を受け取ると，`gramian.computeW`によって$W_1,\ldots,W_n$を計算する． \
...
---
- 注意：
  - コンストラクタ内部で`gramian.computeW`を実行し，$W_1,\ldots,W_n$を計算するため，大規模問題の場合は計算時間のボトルネックになる．
---
---
- 使用例：
...
実際に$W_1,\ldots,W_n$を計算する際には`gramian.computeW`を呼ぶ． \
...
- プロパティの依存関係：
  - MethodとSteps
    ...
    - Method="integral"かつSteps=0でgramian.computeWを実行しようとするとエラー
  - UseScalingとEigTol
...
その際に，`gramian.computeW`によって$W_1,\ldots,W_n$を計算する．
