function varargout = bothcs(A, varargin)
    % BOTHCS Solve the VCS and AECS problem from A and T.
    %
    %   [pV,pA] = bothcs(A)
    ...
    if ~ismember(nargout, allowed)
        error("bothcs:InvalidNargout", "bothcs supports %s outputs.", mat2str(allowed));
    end
    ...
    addParameter(parser, 'UseScaling', [], ...
                 ...
                 (isnumeric(v) && isscalar(v) && any(v == [0 1])));
    ...
    validateattributes(A, {'double'}, {'2d', 'real', 'square'}, 'bothcs', 'A', 1);
    ...
    % WOptions
    if ~isempty(r.Method)
        csopts.Method = r.Method;
    end
    if ~isempty(r.Steps)
        csopts.Steps = r.Steps;
    end
    if ~isempty(r.UseScaling)
        csopts.UseScaling = logical(r.UseScaling);
    end
    if ~isempty(r.EigTol)
        csopts.EigTol = r.EigTol;
    end

    % SolverOptions
    if ~isempty(r.StepSize)
        csopts.StepSize = r.StepSize;
    end
    if ~isempty(r.StepSizeInf)
        csopts.StepSizeInf = r.StepSizeInf;
    end
    if ~isempty(r.MaxIter)
        csopts.MaxIter = r.MaxIter;
    end
    if ~isempty(r.Tol)
        csopts.Tol = r.Tol;
    end
    ...
    if ~isempty(r.Sigma)
        csopts.Sigma = r.Sigma;
    end
    if ~isempty(r.Verbose)
        csopts.Verbose = logical(r.Verbose);
    end
    if ~isempty(r.StoreTrace)
        csopts.StoreTrace = logical(r.StoreTrace);
    end
    ...
    [pA, infoA] = prob.solveAecs(csopts.SolverOptions);

    switch nargout
        case {0, 2}
            varargout = {pV, pA};
        case 4
            varargout = {pV, pA, infoV, infoA};
    end
end
