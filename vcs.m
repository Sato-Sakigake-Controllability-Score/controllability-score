function varargout = vcs(A, varargin)
    % VCS Solve the VCS problem from A and T.
    %
    %   p = vcs(A)
    %   [p,info] = vcs(A,T,...)
    %
    ...
    if ~ismember(nargout, allowed)
        error("vcs:InvalidNargout", "vcs supports %s outputs.", mat2str(allowed));
    end
    ...
    validateattributes(A, {'double'}, {'2d', 'real', 'square'}, 'vcs', 'A', 1);
    ...
    % ---- Solve ----
    [p, info] = prob.solveVcs(csopts.SolverOptions);
